...
// ─── Auth helper (matches profile.tsx / orders list pattern) ──────────────────
...
type OrderDetailResponse =
  | { success: true;  data: { order: OrderDetail; items: OrderItem[] } }
...
function fallback(v: string | null | undefined, placeholder = '—'): string {
  return v?.trim() || placeholder;
}
...
const ORDER_STEPS: { key: string; label: string; icon: React.ReactNode }[] = [
...
function OrderProgress({ status }: { status: string }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-background shadow-sm px-6 py-5 space-y-4">
      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
        Order Progress
      </p>
      ...
      {/* Pending-specific hint — shown until admin confirms the order */}
      {isPending && (
        <div className="flex items-start gap-2 rounded-xl border border-yellow-300/40 bg-yellow-500/5 px-4 py-3">
          <AlertCircle
            className="h-3.5 w-3.5 shrink-0 mt-0.5 text-yellow-600 dark:text-yellow-400"
            strokeWidth={1.75}
          />
          <p className="text-[11px] leading-relaxed text-yellow-700 dark:text-yellow-300">
            Your order has been placed and is awaiting admin review and confirmation.
...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (!userEmail) {
        // Not authenticated — redirect home so the login dialog can open
        router.replace('/');
        return;
      }
      ...
      <div className="min-h-screen bg-muted/30">
        <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 space-y-6">
          ...
          <div className="rounded-2xl border border-destructive/30 bg-destructive/5 px-6 py-8 flex flex-col items-center gap-3 text-center">
            ...
            <Button
              id="order-detail-error-back-btn"
              variant="outline"
              onClick={() => router.replace('/orders')}
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 space-y-6">
        ...
        {/* ── Order progress — read-only, reflects admin updates ── */}
        <OrderProgress status={order.status} />

        {/* ── Cancelled notice ── */}
        {order.status === 'cancelled' && (
          <div className="flex items-start gap-2 rounded-2xl border border-destructive/30 bg-destructive/5 px-5 py-4">
            <X className="h-4 w-4 shrink-0 mt-0.5 text-destructive/70" strokeWidth={1.75} />
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <Section title="Order Info">
            <Row label="Tracking #"  value={fallback(order.trackingNumber)} mono />
            <Row label="Reference #" value={fallback(order.referenceNumber)} mono />
          ...
          <div className="mt-3 space-y-2">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <StatusBadge
                  status={order.downpaymentStatus}
                  map={PAYMENT_STATUS_COLORS}
                />
              </div>
