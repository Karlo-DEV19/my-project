...
interface CheckoutFormProps {
    ...
    clearCart: () => void
    totals: CartTotals
    /** Called whenever the delivery city or province field changes. */
    onAddressChange?: (address: { city: string; province: string }) => void
}
...
export function CheckoutForm({ items, clearCart, totals, onAddressChange }: CheckoutFormProps) {
    ...
    // ── Bubble city/province up so sibling OrderSummary can compute delivery fee ─
    const watchedCity     = useWatch({ control: form.control, name: "address.city" })
    const watchedProvince = useWatch({ control: form.control, name: "address.province" })

    useEffect(() => {
        onAddressChange?.({ city: watchedCity ?? "", province: watchedProvince ?? "" })
    }, [watchedCity, watchedProvince, onAddressChange])
